...
description: "Use this skill when creating a new Phaser 4 game instance or configuring GameConfig options. Covers renderer selection, canvas setup, scaling, pixel art, FPS settings, boot sequence, and all config sub-objects. Triggers on: new Phaser.Game, GameConfig, game setup, renderer, pixel art, FPS. (Phaser 4.2.1 engine reference, vendored from node_modules/phaser/skills/game-setup-and-config — do not edit; run `yarn skills`.)"
---
