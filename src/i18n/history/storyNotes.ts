import type { HistoryCatalog } from './index';

/**
 * The episode behind each Chronicle story: what happened, then what we made of it.
 *
 * The editorial rule is the one the Chronicle was designed under — history has to yield a
 * *behaviour*, not a name. If a source only gave us a name it stayed out, and the second half of
 * each entry is where we say out loud what the game changed. A story with no note here still lists
 * on the page; it shows that its note is not written yet, which is true and better than silence.
 */

export const storyNotesEn: HistoryCatalog = {
  'stories.reed-banner.happened':
    'Đinh Bộ Lĩnh grew up herding buffalo in Hoa Lư and playing at war with the other boys under banners of reed flowers. He put down the twelve warlords who had carved up the country after Ngô Quyền died, and founded Đại Cồ Việt in 968.',
  'stories.reed-banner.inGame':
    'A man who becomes what you make him. The same fragments lead to a founding or to a rebellion, and which one you get depends on how you used him — not on a branch you picked.',

  'stories.goose-feathers.happened':
    'An Dương Vương\u2019s daughter Mỵ Châu married Trọng Thủy, son of the enemy. He found where the crossbow\u2019s trigger was kept and swapped it for a forgery. Fleeing the fall of Cổ Loa, she dropped goose feathers to mark the road — for him.',
  'stories.goose-feathers.inGame':
    'A gift that has been reading you back. The feather beat only exists if the marked hero has been governing a border province, and it fires on his schedule rather than yours.',

  'stories.river-stakes.happened':
    'Twice — in 938 against the Southern Han and in 1288 against the Yuan — a fleet was let into the Bạch Đằng on the flood tide and met by stakes and fire on the ebb. The second time it was the same river, the same trick, and the invaders knew the story.',
  'stories.river-stakes.inGame':
    'A plan with a tide in it. The preparation is slow and visible; the window is short and not yours to move. Miss it and the work is still spent.',

  'stories.dien-hong.happened':
    'Facing the second Mongol invasion in 1284, the retired emperor summoned the elders of the realm to the Diên Hồng hall and asked them whether to fight or come to terms. They shouted to fight.',
  'stories.dien-hong.inGame':
    'A card you do not answer. You present the options; the elders decide, weighted by province loyalty and court stability. Ten quiet minutes of governing cash out in one moment you only watch.',

  'stories.orange.happened':
    'Trần Quốc Toản was too young for the war council at Bình Than. Standing outside, he crushed an orange in his fist without noticing. He raised his own banner, embroidered with six words, and died in the fighting at sixteen.',
  'stories.orange.inGame':
    'Refusal is not a null input. Send him away and he acts anyway — a host you do not control, and an outcome entirely out of your hands.',

  'stories.granaries.happened':
    'Hồ Quý Ly capped how much land any household could hold and how many bondservants it could keep, then took the surplus into state granaries. The reform was right on paper and cost him the country in seven years.',
  'stories.granaries.inGame':
    'Attribution. The story does not cause the famine; it makes sure the famine is remembered as yours.',

  'stories.sixty-five-citadels.happened':
    'The Trưng sisters raised the whole delta against Han rule in 40 AD and held sixty-five citadels for three years. When Ma Yuan came back with the legions Trưng Trắc would not be taken, and went into the Hát river.',
  'stories.sixty-five-citadels.inGame':
    'A rising that gives you far more than you can hold. The lands come fast and the cost of keeping them arrives later, on a schedule you did not set.',

  'stories.ride-the-wind.happened':
    'Bà Triệu rode out in 248 at twenty-three. "I want to ride the strong winds, tread the fierce waves, drive out the invaders — not bend my back to be any man\u2019s concubine."',
  'stories.ride-the-wind.inGame':
    'A hero with an ambition that is not yours. She refuses postings. Refuse her back and she leaves — and the story does not end when she does.',

  'stories.substitution.happened':
    'At Chí Linh in 1419, with Lê Lợi surrounded, Lê Lai put on his lord\u2019s robes, rode out declaring himself the prince of Lam Sơn, and was taken and killed. The rebellion survived because the Ming believed they had already won.',
  'stories.substitution.inGame':
    'A hero you choose to lose. The game will not spend him for you and will not tell you the price is worth it.',

  'stories.borrowed-sword.happened':
    'The legend says a spirit lent Lê Lợi a sword to drive out the Ming, and that afterwards a golden turtle rose in the lake at Thăng Long and took it back. The lake has been called Hoàn Kiếm — returned sword — ever since.',
  'stories.borrowed-sword.inGame':
    'Power on loan, with a due date you are not shown. What it buys is real; keeping it past the return is the part that costs.',

  'stories.slandered.happened':
    'Yi Sun-sin was stripped of command in 1597 on a court accusation, demoted to common soldier, and recalled after the fleet he had built was destroyed without him. He won at Myeongnyang with thirteen ships.',
  'stories.slandered.inGame':
    'Not every story here is ours. This one is Korean, kept because the behaviour is universal: a court that can lose you a war without fighting one.',

  'stories.trusted.happened':
    'Oda Nobunaga sent Akechi Mitsuhide to reinforce an ally in 1582. Mitsuhide turned the army around, marched on Honnō-ji, and killed him there. He held the country for thirteen days.',
  'stories.trusted.inGame':
    'Also not ours. It is in the Chronicle because a subordinate who defects has to be a person you had reasons to trust, or the betrayal is only a dice roll.',

  'stories.assembly.happened':
    'The same Diên Hồng assembly of 1284, told from the floor rather than the throne: what the elders were actually being asked to accept, and what it cost the provinces that answered.',
  'stories.assembly.inGame':
    'The second telling exists because one moment of consent is worth more when you have seen what the consenting cost.',

  'stories.rice-riot.happened':
    'The Nika riots of 532 began at the chariot races and nearly ended Justinian\u2019s reign. The city burned for five days over bread, spectacle, and two factions who had stopped being about racing.',
  'stories.rice-riot.inGame':
    'A capital that turns on you for being rich. Prosperity is not neutral: the grain that made your treasury is the grain the city can see you holding.',

  'stories.no-heir.happened':
    'Lý Chiêu Hoàng was crowned at seven, married at seven, and made to hand the throne to her husband at eight. A dynasty ended because it produced no son, and the men around the child arranged the rest.',
  'stories.no-heir.inGame':
    'A succession that is decided by everyone except the person on the throne.',

  'stories.eat-together.happened':
    'Trần Hưng Đạo\u2019s Hịch tướng sĩ reminded the officers that when they had no clothes he gave them his own, and when they had no rice they ate together. That is the whole argument for why they should now die where he told them to.',
  'stories.eat-together.inGame':
    'Loyalty is a ledger you have been keeping without noticing. What the officers do in a crisis is drawn from what you did in the years before it.',

  'stories.unpaid.happened':
    'Armies that are not paid go home, or worse, stay. Every dynasty here learned it: the Trần paid in land, the Lê in office, the Nguyễn in salt and rice, and the ones that paid in promises kept the throne for a shorter time.',
  'stories.unpaid.inGame':
    'A host that costs money you do not have does not simply vanish from the ledger. It makes a decision of its own.',

  'stories.five-days.happened':
    'Quang Trung marched from Phú Xuân to Thăng Long in forty days, arrived on the fifth night of Tết 1789, and had beaten a Qing army several times his size by the seventh. He had promised his men they would eat their new-year meal in the capital.',
  'stories.five-days.inGame':
    'A window that closes. The march is paid for in the condition of the army that arrives, and the game will let you arrive too tired to win.',

  'stories.ghost-south.happened':
    'Taken alive in 1285 and offered a princedom in the north, Trần Bình Trọng answered that he would rather be a ghost in the south than a king in the north. He was executed.',
  'stories.ghost-south.inGame':
    'A hero taken alive, and an offer made to you rather than to him. Whatever you decide, he has already decided.',

  'stories.without-slaughter.happened':
    'Nguyễn Trãi argued that a city taken by letter costs nothing to hold and a city taken by storm has to be held forever. Several Ming garrisons opened their gates to a piece of paper, and in 1427 the defeated army was given horses and boats to go home in.',
  'stories.without-slaughter.inGame':
    'Victory that is cheaper than it looks, and slower than you want. The letters take turns you would rather spend on the assault.',

  'stories.delayer.happened':
    'Fabius Maximus refused to give Hannibal the battle he needed after Cannae, and was mocked for it in Rome until the strategy worked. Not ours either — kept because the record of it is a record of being hated for being right.',
  'stories.delayer.inGame':
    'A general whose correct advice costs you standing every time you take it.',

  'stories.thanh-giong.happened':
    'A child who had not spoken for three years asked for an iron horse and an iron rod when the invaders came, grew to a giant, drove them out, and rode into the sky at Sóc Sơn without waiting to be thanked.',
  'stories.thanh-giong.inGame':
    'Help that arrives from nowhere, does exactly one thing, and cannot be kept, promoted or asked for again.',

  'stories.mountain-water.happened':
    'Sơn Tinh and Thủy Tinh both asked for the same princess. The mountain arrived first. The water has been coming back every year since, and that is the flood season.',
  'stories.mountain-water.inGame':
    'A rival who does not need to win once. Losing to you is the reason it returns on a schedule.',

  'stories.salt-road.happened':
    'Salt came up from the coast to the highlands and rice came down, along roads nobody garrisoned because nobody was fighting over them. Cut one and two provinces starve without a battle being fought.',
  'stories.salt-road.inGame':
    'The part of the map you stopped watching. Nothing announces it until the effect arrives somewhere else.',

  'stories.thirteenth.happened':
    'Twelve warlords carved up the country after 944 and are counted in every chronicle. The thirteenth is the one who was too small to be listed, and the point of the story is that the list is not the country.',
  'stories.thirteenth.inGame':
    'A rival that comes apart. What replaces it is not on any of the lists you have been reading.',

  'stories.sickness.happened':
    'Plague and cholera moved along the same rivers and roads the grain did, and the dynasties that survived one were the ones with granaries and dyke corvée already standing.',
  'stories.sickness.inGame':
    'A crisis that cannot be fought, only prepared for, and that arrives on a schedule indifferent to whatever else you were doing.',

  'stories.cham-engineer.happened':
    'Đại Việt and Champa fought for six centuries and borrowed from each other the whole time — towers, music, rice strains, and the siege engineering that appears in Việt armies not long after it appears in Chăm ones.',
  'stories.cham-engineer.inGame':
    'Expertise that arrives from the people you are at war with, and a court that would rather not be reminded where it came from.',

  'stories.counting-house.happened':
    'Nine hundred taels is a number out of the tax registers, not out of an epic. Most of what a dynasty did was arithmetic, and most of what brought one down was arithmetic that had been quietly wrong for years.',
  'stories.counting-house.inGame':
    'A crisis that begins in a ledger. There is no battle in it anywhere.',

  'stories.dai-cao.happened':
    'Nguyễn Trãi\u2019s Bình Ngô đại cáo, 1428, announced the Ming defeat and opened by stating that Đại Việt has its own culture, its own mountains and rivers, and its own line of emperors — an argument about what a country is, made at the moment of winning one.',
  'stories.dai-cao.inGame':
    'A charge you accept or decline. Taking it commits you to finishing the war a particular way, and pays in a card that exists nowhere else.',

  'stories.chieu-doi-do.happened':
    'The Chiếu dời đô of 1010 gave reasons for moving the capital out of the mountains: Hoa Lư was defensible and poor, Đại La was flat, central, and where a country could actually be governed from.',
  'stories.chieu-doi-do.inGame':
    'A move that costs you your safest position on purpose, and cannot be undone once the court has followed you.',

  'stories.nam-quoc.happened':
    'In 1076, with the Song across the Như Nguyệt, four lines were read aloud from a temple at night: the southern realm is the southern emperor\u2019s, so written in the Book of Heaven. Authorship is disputed; the effect on the men who heard it is not.',
  'stories.nam-quoc.inGame':
    'Morale as a resource you can spend once. What it buys is a line that holds one more day than it should.',

  'stories.hich-tuong-si.happened':
    'Trần Hưng Đạo wrote to his officers around 1284 that he forgot to eat and could not sleep, and asked them what they would say when the country was gone and their own names went with it. The Hịch was copied out and read to the ranks.',
  'stories.hich-tuong-si.inGame':
    'A charge that spends your own standing rather than your treasury, and cannot be taken twice by the same court.',

  'stories.chi-lang.happened':
    'In 1427 the Ming relief army under Liễu Thăng was drawn into the Chi Lăng pass and destroyed there. The siege it was marching to lift ended by negotiation a few weeks later.',
  'stories.chi-lang.inGame':
    'The trap that only works if you are patient enough to leave the thing being besieged unrelieved while you set it.',

  'stories.than-toc.happened':
    'Thần tốc — divine speed. Quang Trung\u2019s men marched in relays, three carrying one in a hammock so the column never stopped, and covered six hundred kilometres in forty days.',
  'stories.than-toc.inGame':
    'Speed bought with condition. The army that arrives is not the army that set out, and the game makes you look at the difference.',

  'stories.tien-phat.happened':
    'Tiên phát chế nhân — strike first to master the enemy. In 1075 Lý Thường Kiệt crossed into Song territory and burned the depots at Ung Châu that were being stocked for an invasion of him.',
  'stories.tien-phat.inGame':
    'A pre-emptive war you have to justify afterwards to a court that did not see the depots.',

  'stories.hai-ba-trung.happened':
    'Trưng Trắc and Trưng Nhị, 40–43. The rising began after the Han governor executed Trưng Trắc\u2019s husband, and the record is unusually clear that the officers under them were largely women.',
  'stories.hai-ba-trung.inGame':
    'A charge taken in grief. It gives you more than you can hold, and the game does not let you give any of it back.',

  'stories.ho-guom.happened':
    'The sword lent to Lê Lợi and taken back by a golden turtle in the lake at Thăng Long, 1428. The lake still carries the name, and the story is the country explaining to itself that the mandate was borrowed, not owned.',
  'stories.ho-guom.inGame':
    'What was lent has to be returned, and the game picks the moment.',

  'stories.no-than.happened':
    'The magic crossbow of Cổ Loa, its trigger made from a golden turtle\u2019s claw, and the son-in-law who replaced it with a copy. Archaeology has turned up bronze trigger mechanisms and tens of thousands of arrowheads at the site; the turtle is the part the record adds.',
  'stories.no-than.inGame':
    'A weapon that stops working, and no message anywhere telling you when it stopped.',

  'stories.yet-kieu.happened':
    'A fisherman\u2019s son who could stay under water most of a day. He went out at night with an auger and put a section of the Yuan fleet on the riverbed without a battle being fought.',
  'stories.yet-kieu.inGame':
    'A verb the game did not have: damage done to a fleet by one man, at night, with no engagement to resolve.',

  'stories.van-don.happened':
    'Trần Khánh Dư, disgraced and reduced to selling charcoal on the coast, was recalled in 1288 and took the entire Yuan grain fleet at Vân Đồn. The army it was feeding had already won its battles and starved anyway.',
  'stories.van-don.inGame':
    'The supply line as the target. Winning every fight and losing the campaign is a thing the game will let happen to either side.',

  'stories.paper-money.happened':
    'Thông bảo hội sao, 1400: the country\u2019s first paper currency, backed by a law that made holding bronze coin a crime. Counterfeiting began at once and the notes were worthless within a few years.',
  'stories.paper-money.inGame':
    'A reform that works in the ledger and fails in the market, on a delay long enough that you will have moved on before it lands.',

  'stories.luy-thay.happened':
    'Đào Duy Từ was barred from the examinations because his father had been a stage performer, a trade the Lê code ruled out of the halls. He went south instead and built the Nguyễn their walls at Đồng Hới in 1631. They held the Trịnh out for the better part of fifty years.',
  'stories.luy-thay.inGame':
    'A defence built by a man the other side rejected. What it costs is everything you were not building instead.',

  'stories.van-mieu.happened':
    'The Văn Miếu was founded in 1070 and the Quốc Tử Giám in 1076. The doctoral stelae still standing in Hà Nội are a list of names of men from villages that would otherwise be in no record at all.',
  'stories.van-mieu.inGame':
    'An institution that pays out later than any player is comfortable with, and keeps paying long after the reign that built it.',

  'stories.thu-do.happened':
    'Asked in 1258 whether the court should come to terms with the Mongols, Trần Thủ Độ answered that his head must fall first. It did not.',
  'stories.thu-do.inGame':
    'A minister whose certainty is a resource, and whose certainty is occasionally wrong.',

  'stories.binh-trong.happened':
    'The same capture as the ghost-in-the-south telling, from the other end: what the offer looked like to the men who made it, and why a refusal that changed nothing militarily was recorded anyway.',
  'stories.binh-trong.inGame':
    'A hero the enemy would rather have than kill, and a decision that is yours in name only.',

  'stories.khuc-thua-du.happened':
    'In 905, with the Tang collapsing, Khúc Thừa Dụ took the governorship at Đại La and asked to be confirmed in it. He was. Autonomy arrived by administration where three centuries of rebellion had not managed it.',
  'stories.khuc-thua-du.inGame':
    'Independence won without a war, and a court that will not count it as a victory.',

  'stories.the-dykes.happened':
    'The Red River runs above its own delta and is held there by earth. Every dynasty ran a dyke corvée; the Trần created an office for it. A season of neglect is a famine two years later.',
  'stories.the-dykes.inGame':
    'Maintenance. The only system in the game whose reward for working perfectly is that nothing at all happens.',

  'stories.che-bong-nga.happened':
    'The Cham king Chế Bồng Nga sacked Thăng Long three times between 1371 and 1383. The Trần could not stop him and did not beat him; he was killed in 1390 when a defector pointed out his ship to the Việt gunners.',
  'stories.che-bong-nga.inGame':
    'An enemy who keeps coming back stronger, and who is not removed by anything you would call a victory.',
};

export const storyNotesVi: HistoryCatalog = {
  'stories.reed-banner.happened':
    'Đinh Bộ Lĩnh lớn lên chăn trâu ở Hoa Lư, cùng lũ trẻ trong vùng lấy bông lau làm cờ bày trận. Sau khi Ngô Quyền mất, nước chia làm mười hai sứ quân; ông dẹp xong cả mười hai, rồi năm 968 lên ngôi, đặt quốc hiệu Đại Cồ Việt.',
  'stories.reed-banner.inGame':
    'Một người trở thành đúng cái ngươi làm nên. Cùng những mảnh chuyện ấy dẫn tới một cuộc lập quốc hay một cuộc phản loạn, và rơi vào bên nào là do ngươi đã dùng ông ta thế nào, chứ không do một nhánh ngươi bấm chọn.',

  'stories.goose-feathers.happened':
    'Mỵ Châu, con gái An Dương Vương, lấy Trọng Thủy là con của kẻ thù. Chàng tìm ra chỗ cất lẫy nỏ và tráo bằng một cái giả. Khi Cổ Loa thất thủ, nàng theo cha chạy về nam, dọc đường rắc lông ngỗng đánh dấu — đánh dấu cho chàng.',
  'stories.goose-feathers.inGame':
    'Món quà vẫn đang đọc ngược lại ngươi. Nhịp lông ngỗng chỉ có nếu người được đánh dấu đang trấn một châu ngoài biên, và nó nổ theo lịch của ông ta chứ không theo lịch của ngươi.',

  'stories.river-stakes.happened':
    'Hai lần — năm 938 với quân Nam Hán và năm 1288 với quân Nguyên — một hạm đội được dử vào sông Bạch Đằng lúc nước lớn, rồi gặp cọc và gặp lửa lúc nước ròng. Lần sau vẫn con sông ấy, vẫn cái kế ấy, và bên kia đã biết trước cả tích này.',
  'stories.river-stakes.inGame':
    'Một mưu kế có con nước trong đó. Việc chuẩn bị thì chậm và lộ; khoảng hở thì hẹp, và không phải thứ ngươi dời được. Lỡ một nhịp thì công sức vẫn cứ mất.',

  'stories.dien-hong.happened':
    'Trước cuộc xâm lăng lần thứ hai của quân Nguyên, năm 1284 Thượng hoàng vời các bô lão trong nước về điện Diên Hồng, hỏi nên hoà hay nên đánh. Muôn miệng cùng hô: đánh.',
  'stories.dien-hong.inGame':
    'Một lá bài mà ngươi không trả lời. Ngươi bày ra các lối chọn; các bô lão quyết, nặng nhẹ theo lòng dân các châu và theo sự yên ổn của triều đình. Mười phút cai trị lặng lẽ được thanh toán trong một khoảnh khắc mà ngươi chỉ đứng nhìn.',

  'stories.orange.happened':
    'Trần Quốc Toản còn nhỏ, không được dự hội nghị Bình Than. Đứng ngoài, cậu bóp nát quả cam trong tay mà không hay. Cậu về dựng cờ riêng thêu sáu chữ, và tử trận năm mười sáu tuổi.',
  'stories.orange.inGame':
    'Từ chối không phải là một lựa chọn bỏ trống. Đuổi cậu về thì cậu vẫn cứ làm — một đạo quân ngươi không điều khiển, và một kết cục hoàn toàn ngoài tay ngươi.',

  'stories.granaries.happened':
    'Hồ Quý Ly hạn định số ruộng mỗi nhà được giữ và số gia nô được nuôi, rồi thu phần dư vào kho nhà nước. Cải cách đúng trên giấy, và bảy năm sau thì mất nước.',
  'stories.granaries.inGame':
    'Chuyện quy công. Tích này không gây ra nạn đói; nó chỉ lo cho nạn đói ấy được nhớ là của ngươi.',

  'stories.sixty-five-citadels.happened':
    'Năm 40, Hai Bà Trưng dấy binh chống nhà Hán, cả châu thổ hưởng ứng, lấy được sáu mươi lăm thành và giữ trong ba năm. Đến khi Mã Viện đem quân sang lần nữa, Trưng Trắc không chịu để bị bắt, gieo mình xuống sông Hát.',
  'stories.sixty-five-citadels.inGame':
    'Một cuộc dấy binh cho ngươi nhiều hơn sức ngươi giữ. Đất về rất nhanh, còn cái giá để giữ thì đến sau, theo một cái lịch không do ngươi đặt.',

  'stories.ride-the-wind.happened':
    'Năm 248, Bà Triệu ra trận, bấy giờ hai mươi ba tuổi. "Tôi muốn cưỡi cơn gió mạnh, đạp luồng sóng dữ, chém cá kình ở biển Đông, đánh đuổi quân Ngô, giành lại giang sơn, cởi ách nô lệ, chứ không chịu khom lưng làm tì thiếp cho người."',
  'stories.ride-the-wind.inGame':
    'Một người hùng mang chí riêng, và cái chí ấy không phải chí của ngươi. Bà từ chối các chức phong. Ngươi từ chối lại thì bà bỏ đi — và câu chuyện không kết thúc khi bà đi.',

  'stories.substitution.happened':
    'Năm 1419 ở Chí Linh, khi Lê Lợi bị vây, Lê Lai mặc áo bào chúa, xưng là chúa Lam Sơn mà xông ra, rồi bị bắt và bị giết. Cuộc khởi nghĩa sống được là vì quân Minh tin rằng mình đã thắng.',
  'stories.substitution.inGame':
    'Một người hùng mà ngươi chọn để mất. Trò chơi sẽ không tự tiêu ông ta thay ngươi, và cũng không nói cho ngươi biết rằng cái giá ấy là đáng.',

  'stories.borrowed-sword.happened':
    'Truyền rằng thần cho Lê Lợi mượn gươm để đuổi giặc Minh, và sau đó rùa vàng nổi lên ở hồ giữa Thăng Long đòi lại. Hồ mang tên Hoàn Kiếm từ bấy giờ.',
  'stories.borrowed-sword.inGame':
    'Quyền lực đi mượn, với một kỳ hạn không ai cho biết. Cái nó mua được là thật; giữ quá hạn mới là phần phải trả.',

  'stories.slandered.happened':
    'Năm 1597, Yi Sun-sin — chữ Hán đọc là Lý Thuấn Thần — bị tước binh quyền vì một lời tố ở triều, giáng làm lính trơn, rồi được gọi lại sau khi hạm đội chính ông dựng nên bị diệt lúc vắng ông. Ông thắng ở Myeongnyang với mười ba chiếc thuyền.',
  'stories.slandered.inGame':
    'Không phải tích nào ở đây cũng là của ta. Tích này của Triều Tiên, giữ lại vì cái nết ấy ở đâu cũng có: một triều đình có thể làm ngươi thua cả một cuộc chiến mà không cần đánh trận nào.',

  'stories.trusted.happened':
    'Năm 1582 Oda Nobunaga sai Akechi Mitsuhide đem quân đi tiếp viện cho một đồng minh. Mitsuhide quay quân, kéo về chùa Honnō và giết ông ở đó. Y giữ được thiên hạ mười ba ngày.',
  'stories.trusted.inGame':
    'Cũng không phải của ta. Nó có mặt trong Sử Ký vì một thuộc tướng phản phải là người ngươi từng có lý do để tin; không thì sự phản bội chỉ còn là một con xúc xắc.',

  'stories.assembly.happened':
    'Vẫn hội nghị Diên Hồng năm 1284, nhưng kể từ dưới sân chứ không từ trên ngai: các bô lão thật ra đang được hỏi rằng họ sẽ phải gánh những gì, và các châu đã trả bao nhiêu cho tiếng "đánh" ấy.',
  'stories.assembly.inGame':
    'Lần kể thứ hai có mặt vì một khoảnh khắc đồng lòng đáng giá hơn khi ngươi đã thấy sự đồng lòng ấy tốn những gì.',

  'stories.rice-riot.happened':
    'Loạn Nika năm 532 khởi từ trường đua ngựa và suýt kết liễu triều Justinian. Kinh thành cháy năm ngày vì miếng bánh, vì trò vui, và vì hai phe đã từ lâu không còn là chuyện đua ngựa nữa.',
  'stories.rice-riot.inGame':
    'Một kinh đô trở mặt với ngươi vì ngươi giàu. Thịnh vượng không phải thứ trung tính: thóc làm nên kho bạc của ngươi cũng là thóc mà cả thành nhìn thấy ngươi đang giữ.',

  'stories.no-heir.happened':
    'Lý Chiêu Hoàng lên ngôi năm bảy tuổi, lấy chồng năm bảy tuổi, và bị ép nhường ngôi cho chồng năm tám tuổi. Một triều đại chấm dứt vì không sinh được con trai, còn những người đàn ông quanh đứa bé thì sắp đặt nốt phần còn lại.',
  'stories.no-heir.inGame':
    'Một cuộc kế vị do tất cả mọi người quyết, trừ chính người đang ngồi trên ngai.',

  'stories.eat-together.happened':
    'Hịch tướng sĩ của Trần Hưng Đạo nhắc các tướng rằng khi họ không có áo thì ông cho áo mình, khi không có cơm thì ăn cùng một mâm. Đó là toàn bộ lý lẽ cho việc bây giờ họ phải chết ở nơi ông chỉ.',
  'stories.eat-together.inGame':
    'Lòng trung là một cuốn sổ ngươi đã ghi mà không để ý. Việc các tướng làm lúc nguy được rút ra từ việc chính ngươi đã làm những năm trước đó.',

  'stories.unpaid.happened':
    'Quân không được trả công thì bỏ về, hoặc tệ hơn, ở lại. Triều nào ở đây cũng học được điều đó: nhà Trần trả bằng ruộng, nhà Lê bằng chức, nhà Nguyễn bằng muối và gạo; còn triều nào trả bằng lời hứa thì giữ ngai ngắn hơn.',
  'stories.unpaid.inGame':
    'Một đạo quân tốn thứ tiền ngươi không có sẽ không lặng lẽ biến mất khỏi sổ sách. Nó tự ra lấy một quyết định của riêng nó.',

  'stories.five-days.happened':
    'Quang Trung đem quân từ Phú Xuân ra Thăng Long trong bốn mươi ngày, đến vào đêm mồng năm Tết Kỷ Dậu 1789, và đến mồng bảy đã phá xong đạo quân Thanh đông gấp nhiều lần. Ông đã hứa với quân sĩ rằng họ sẽ ăn Tết trong kinh thành.',
  'stories.five-days.inGame':
    'Một cánh cửa đang khép. Cuộc hành quân được trả bằng tình trạng của đạo quân lúc tới nơi, và trò chơi cho phép ngươi tới nơi mệt đến mức không thắng nổi.',

  'stories.ghost-south.happened':
    'Bị bắt sống năm 1285 và được dụ phong vương đất Bắc, Trần Bình Trọng đáp: "Ta thà làm quỷ nước Nam, chứ không thèm làm vương đất Bắc." Ông bị giết.',
  'stories.ghost-south.inGame':
    'Một người hùng bị bắt sống, và một lời dụ đưa cho ngươi chứ không đưa cho ông ta. Ngươi quyết thế nào thì ông ta cũng đã quyết xong rồi.',

  'stories.without-slaughter.happened':
    'Nguyễn Trãi cho rằng một thành lấy bằng thư thì giữ không tốn gì, còn một thành lấy bằng máu thì phải giữ mãi mãi. Nhiều đồn quân Minh mở cổng cho một tờ giấy, và năm 1427 đạo quân bại trận được cấp ngựa và thuyền cho về nước.',
  'stories.without-slaughter.inGame':
    'Chiến thắng rẻ hơn vẻ ngoài của nó, và chậm hơn ngươi muốn. Những bức thư ăn mất số lượt mà ngươi thà dùng để công thành.',

  'stories.delayer.happened':
    'Sau trận Cannae, Fabius Maximus nhất định không cho Hannibal cái trận mà Hannibal cần, và bị cả La Mã chê cười cho đến khi phương lược ấy thành công. Cũng không phải chuyện của ta — giữ lại vì đây là bản ghi xưa nhất về việc bị ghét chỉ vì mình đúng.',
  'stories.delayer.inGame':
    'Một vị tướng mà mỗi lần ngươi nghe theo lời khuyên đúng của ông ta là một lần ngươi mất uy tín.',

  'stories.thanh-giong.happened':
    'Một đứa bé ba năm không nói, khi giặc Ân sang thì xin ngựa sắt roi sắt, vươn vai thành người khổng lồ, đuổi xong giặc rồi bay lên trời ở Sóc Sơn mà không đợi ai cảm ơn.',
  'stories.thanh-giong.inGame':
    'Sự trợ giúp đến từ hư không, làm đúng một việc, rồi không giữ lại được, không thăng chức được, cũng không xin thêm lần nữa được.',

  'stories.mountain-water.happened':
    'Sơn Tinh và Thủy Tinh cùng hỏi một người con gái. Thần Núi tới trước. Thần Nước từ đó năm nào cũng quay lại, và đó là mùa nước lũ.',
  'stories.mountain-water.inGame':
    'Một đối thủ không cần thắng lần nào. Chính việc thua ngươi mới là lý do nó trở lại đúng hẹn.',

  'stories.salt-road.happened':
    'Muối từ biển ngược lên miền cao, gạo xuôi xuống, theo những con đường không ai đóng đồn vì không ai tranh. Cắt một con đường ấy là hai châu đói, mà chẳng có trận nào xảy ra.',
  'stories.salt-road.inGame':
    'Phần bản đồ mà ngươi đã thôi để mắt tới. Không có gì báo trước, cho đến khi hậu quả hiện ra ở một nơi khác.',

  'stories.thirteenth.happened':
    'Mười hai sứ quân chia cắt đất nước sau năm 944, và sử nào cũng đếm đủ mười hai. Người thứ mười ba là kẻ nhỏ quá nên không được chép — và ý của tích này nằm ở đó: cái danh sách không phải là đất nước.',
  'stories.thirteenth.inGame':
    'Một đối thủ tan rã. Cái thay chỗ nó không có trong bất cứ danh sách nào ngươi đã đọc.',

  'stories.sickness.happened':
    'Dịch hạch và dịch tả đi theo đúng những con sông và con đường mà thóc gạo đi. Triều nào sống sót qua một trận dịch là triều đã có sẵn kho lẫm và phu đê điều.',
  'stories.sickness.inGame':
    'Một tai hoạ không đánh được, chỉ phòng được, và nó đến theo một cái lịch không đoái hoài tới việc ngươi đang bận gì.',

  'stories.cham-engineer.happened':
    'Đại Việt và Chiêm Thành đánh nhau sáu thế kỷ, và suốt sáu thế kỷ ấy vay mượn của nhau — tháp, nhạc, giống lúa, và cả nghề công thành, thứ xuất hiện trong quân Việt không lâu sau khi xuất hiện trong quân Chăm.',
  'stories.cham-engineer.inGame':
    'Tài nghệ đến từ chính những người ngươi đang đánh, và một triều đình thà không phải nhắc lại nó từ đâu mà ra.',

  'stories.counting-house.happened':
    'Chín trăm lượng là con số lấy ra từ sổ thuế chứ không phải từ một thiên anh hùng ca. Phần lớn việc một triều đại làm là tính toán, và phần lớn cái làm sụp một triều đại là những phép tính đã sai lặng lẽ nhiều năm.',
  'stories.counting-house.inGame':
    'Một cơn nguy bắt đầu trong sổ sách. Trong đó không có trận đánh nào cả.',

  'stories.dai-cao.happened':
    'Bình Ngô đại cáo của Nguyễn Trãi, năm 1428, bố cáo việc quân Minh đã bại, mở đầu bằng lời rằng nước Đại Việt ta có nền văn hiến riêng, có núi sông bờ cõi riêng, mỗi đời đều có đế một phương — một lập luận về thế nào là một nước, nói ra đúng vào lúc vừa giành lại được nước.',
  'stories.dai-cao.inGame':
    'Một lời thề ngươi nhận hoặc từ chối. Nhận là tự buộc mình kết thúc cuộc chiến theo một cách nhất định, và trả bằng một lá bài không có ở đâu khác.',

  'stories.chieu-doi-do.happened':
    'Chiếu dời đô năm 1010 nêu rõ lý do rời kinh khỏi vùng núi: Hoa Lư hiểm mà chật, còn Đại La ở giữa trời đất, đất rộng mà bằng, là nơi có thể thật sự cai trị một nước.',
  'stories.chieu-doi-do.inGame':
    'Một nước cờ cố ý bỏ đi chỗ đứng an toàn nhất của ngươi, và không rút lại được khi triều đình đã theo ngươi đi rồi.',

  'stories.nam-quoc.happened':
    'Năm 1076, khi quân Tống đóng bên kia sông Như Nguyệt, bốn câu thơ được đọc lên trong đền lúc đêm: "Nam quốc sơn hà Nam đế cư, tiệt nhiên định phận tại thiên thư." Ai là tác giả thì đến nay còn bàn; tác động lên những người nghe nó đêm ấy thì không.',
  'stories.nam-quoc.inGame':
    'Sĩ khí như một nguồn lực chỉ tiêu được đúng một lần. Cái nó mua là một phòng tuyến trụ thêm một ngày nữa so với lẽ ra.',

  'stories.hich-tuong-si.happened':
    'Khoảng năm 1284, Trần Hưng Đạo viết cho các tướng dưới quyền rằng ông tới bữa quên ăn, nửa đêm vỗ gối, rồi hỏi họ sẽ nói gì khi nước mất và tên tuổi chính họ cũng mất theo. Bài hịch được chép ra và đọc trước hàng quân.',
  'stories.hich-tuong-si.inGame':
    'Một lời thề tiêu bằng uy tín của chính ngươi chứ không bằng kho bạc, và một triều không thể nhận nó hai lần.',

  'stories.chi-lang.happened':
    'Năm 1427, đạo viện binh nhà Minh do Liễu Thăng cầm đầu bị dụ vào ải Chi Lăng và bị diệt tại đó. Cuộc vây thành mà nó đang đi giải toả kết thúc bằng thương lượng vài tuần sau.',
  'stories.chi-lang.inGame':
    'Cái bẫy chỉ ăn nếu ngươi đủ kiên nhẫn để mặc chỗ đang bị vây không được cứu trong lúc ngươi giăng nó.',

  'stories.than-toc.happened':
    'Thần tốc. Quân Quang Trung đi thay phiên, ba người khiêng một người trên võng để đoàn quân không lúc nào phải dừng, vượt sáu trăm cây số trong bốn mươi ngày.',
  'stories.than-toc.inGame':
    'Tốc độ mua bằng sức lực. Đạo quân tới nơi không còn là đạo quân lúc xuất phát, và trò chơi bắt ngươi nhìn thẳng vào chỗ chênh ấy.',

  'stories.tien-phat.happened':
    'Tiên phát chế nhân. Năm 1075 Lý Thường Kiệt đem quân sang đất Tống, đốt các kho ở Ung Châu đang được chất đầy để chuẩn bị đánh xuống.',
  'stories.tien-phat.inGame':
    'Một cuộc đánh phủ đầu mà sau đó ngươi phải phân trần trước một triều đình chưa hề nhìn thấy những cái kho ấy.',

  'stories.hai-ba-trung.happened':
    'Trưng Trắc và Trưng Nhị, những năm 40–43. Cuộc khởi nghĩa nổ ra sau khi thái thú nhà Hán là Tô Định giết chồng bà Trưng Trắc, và sử chép rõ một cách hiếm thấy rằng các tướng dưới quyền hai bà phần nhiều là đàn bà.',
  'stories.hai-ba-trung.inGame':
    'Một lời thề nhận trong tang tóc. Nó cho ngươi nhiều hơn sức ngươi giữ, và trò chơi không cho ngươi trả lại phần nào.',

  'stories.ho-guom.happened':
    'Thanh gươm cho Lê Lợi mượn, và được rùa vàng đòi lại ở hồ giữa Thăng Long, năm 1428. Hồ vẫn mang cái tên ấy, và tích này là cách đất nước tự giải thích với mình rằng mệnh trời là thứ đi mượn chứ không phải thứ sở hữu.',
  'stories.ho-guom.inGame':
    'Cái đã mượn thì phải trả, và trò chơi chọn lấy thời điểm.',

  'stories.no-than.happened':
    'Nỏ thần Cổ Loa, lẫy làm bằng móng rùa vàng, và người con rể đã tráo nó bằng một cái giả. Khảo cổ đào được lẫy nỏ bằng đồng và hàng vạn mũi tên ngay tại di chỉ; con rùa là phần sử chép thêm.',
  'stories.no-than.inGame':
    'Một thứ vũ khí thôi hoạt động, và không một dòng nào ở đâu báo cho ngươi biết nó thôi từ lúc nào.',

  'stories.yet-kieu.happened':
    'Con nhà chài, lặn được gần trọn một ngày dưới nước. Ông đi đêm với cái khoan, và đưa được một phần hạm đội Nguyên xuống đáy sông mà không phải đánh trận nào.',
  'stories.yet-kieu.inGame':
    'Một động từ mà trò chơi trước đó chưa có: thiệt hại giáng lên cả một hạm đội bởi một người, trong đêm, không có trận nào để phân thắng bại.',

  'stories.van-don.happened':
    'Trần Khánh Dư bị cách chức, phải bán than ngoài bờ biển, rồi được gọi lại và năm 1288 cướp trọn đoàn thuyền lương của quân Nguyên ở Vân Đồn. Đạo quân mà đoàn thuyền ấy nuôi đã thắng hết các trận của mình, rồi vẫn chết đói.',
  'stories.van-don.inGame':
    'Đường lương là một mục tiêu. Thắng mọi trận mà thua cả cuộc chinh phạt là điều trò chơi cho phép xảy ra với cả hai bên.',

  'stories.paper-money.happened':
    'Thông bảo hội sao, năm 1400: đồng tiền giấy đầu tiên của nước ta, dựa vào một đạo luật khép tội người còn giữ tiền đồng. Tiền giả xuất hiện ngay lập tức, và chỉ vài năm sau giấy bạc thành vô giá trị.',
  'stories.paper-money.inGame':
    'Một cải cách chạy đúng trong sổ sách và hỏng ngoài chợ, với độ trễ đủ dài để ngươi đã đi làm việc khác trước khi nó rơi xuống.',

  'stories.luy-thay.happened':
    'Đào Duy Từ bị cấm thi vì cha làm nghề xướng ca, nghề mà lệ triều Lê xếp vào hạng không được ứng thí. Ông bèn vào nam, và năm 1631 đắp cho chúa Nguyễn hệ luỹ ở Đồng Hới. Luỹ ấy chặn quân Trịnh gần năm mươi năm.',
  'stories.luy-thay.inGame':
    'Một phòng tuyến do chính người bị bên kia chối bỏ dựng nên. Cái giá của nó là tất cả những thứ ngươi đã không xây vì đã xây nó.',

  'stories.van-mieu.happened':
    'Văn Miếu dựng năm 1070, Quốc Tử Giám lập năm 1076. Những tấm bia tiến sĩ còn đứng ở Hà Nội là một bảng tên người từ những làng mà nếu không có bia thì chẳng sổ sách nào ghi lại.',
  'stories.van-mieu.inGame':
    'Một thiết chế trả lãi muộn hơn mức bất cứ người chơi nào thấy dễ chịu, và trả mãi rất lâu sau khi cái triều dựng nên nó đã mất.',

  'stories.thu-do.happened':
    'Năm 1258, khi triều đình hỏi có nên hoà với quân Mông Cổ không, Trần Thủ Độ đáp: "Đầu thần chưa rơi xuống đất, xin bệ hạ đừng lo." Đầu ông không rơi.',
  'stories.thu-do.inGame':
    'Một vị đại thần mà sự chắc chắn của ông là một nguồn lực, và thỉnh thoảng sự chắc chắn ấy sai.',

  'stories.binh-trong.happened':
    'Vẫn vụ bắt sống ấy, kể từ đầu bên kia: lời dụ trông ra sao trong mắt chính những người đưa nó ra, và vì sao một lời từ chối không thay đổi được gì về mặt binh bị vẫn cứ được chép lại.',
  'stories.binh-trong.inGame':
    'Một người hùng mà giặc thà có được còn hơn giết đi, và một quyết định chỉ mang tên ngươi.',

  'stories.khuc-thua-du.happened':
    'Năm 905, nhân nhà Đường đang đổ, Khúc Thừa Dụ nhận lấy chức Tiết độ sứ ở Đại La rồi xin được công nhận. Ông được công nhận. Quyền tự chủ về bằng đường hành chính, ở chỗ mà ba thế kỷ khởi nghĩa đã không làm nổi.',
  'stories.khuc-thua-du.inGame':
    'Nền độc lập giành được mà không có chiến tranh, và một triều đình không chịu tính đó là một chiến thắng.',

  'stories.the-dykes.happened':
    'Sông Hồng chảy cao hơn chính châu thổ của nó, và được giữ ở đó bằng đất. Triều nào cũng phải trưng phu đắp đê; nhà Trần lập hẳn chức Hà đê sứ. Bỏ bê một mùa là một nạn đói hai năm sau.',
  'stories.the-dykes.inGame':
    'Chuyện bảo trì. Hệ thống duy nhất trong trò chơi mà phần thưởng cho việc chạy hoàn hảo là không có chuyện gì xảy ra cả.',

  'stories.che-bong-nga.happened':
    'Vua Chiêm là Chế Bồng Nga ba lần đánh vào Thăng Long trong khoảng 1371–1383. Nhà Trần không chặn nổi và cũng không thắng nổi ông; ông chết năm 1390, khi một hàng tướng chỉ đúng chiếc thuyền của ông cho pháo bên quân Việt.',
  'stories.che-bong-nga.inGame':
    'Một kẻ địch cứ quay lại mạnh hơn, và không bị loại bỏ bởi bất cứ thứ gì ngươi gọi là chiến thắng.',
};
